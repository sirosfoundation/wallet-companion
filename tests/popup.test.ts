/**
 * Unit tests for popup.ts - Extension popup UI logic
 */

import type { UsageStats } from '@shared/schemas/resources';
...
describe('Popup - UI State Management', () => {
	...
	describe('updateUI()', () => {
		function updateUI(enabled: boolean, stats: UsageStats | null): void {
			...
			if (!statusDiv || !statusText || !extensionToggle || !interceptCount) return;
			...
			if (stats) {
				interceptCount.textContent = String(stats.interceptCount || 0);
			}
		}
		...
		test('should display 0 when interceptCount is missing', () => {
			updateUI(true, { interceptCount: 0, walletUses: {} });
	...
	describe('displayWallets()', () => {
		...
		function displayWallets(wallets: MockWallet[] | null, _stats: UsageStats | null): void {
			const walletList = document.getElementById('walletList');
			const walletCount = document.getElementById('walletCount');

			if (!walletList || !walletCount) return;
        ...
        `;
			...
			walletCount.textContent = String(enabledWallets.length);
		...
		test('should use default icon when not provided', () => {
			const walletsNoIcon: MockWallet[] = [{ id: 'w1', name: 'No Icon', url: 'https://test.com', enabled: true }];
			displayWallets(walletsNoIcon, { interceptCount: 0, walletUses: {} });
		...
		test('should escape HTML in wallet names', () => {
			const maliciousWallets: MockWallet[] = [
				{
					id: 'w1',
			...
			displayWallets(maliciousWallets, { interceptCount: 0, walletUses: {} });
